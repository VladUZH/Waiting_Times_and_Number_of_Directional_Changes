...
//        ////////////////////////////////////////////////////////////////////
        ...
        String fileName = "C:/Users/Vladimir Petrov/Documents/iPython/MM_square_grid/bigAgentBasedTimeSeries.csv";
        ...
        double sumTimeUp = 0;
        double sumTimeDown = 0;
        long nStepsPerYear = 10000000;

        ATick aTick = new ATick(1.0);
        Runner runner = new Runner(deltaUp, deltaDown, aTick, 1, false);
        ...
        try {
            ...
            while ((line = bufferedReader.readLine()) != null) {
                aTick = new ATick(Double.parseDouble(line));
                int runResult = runner.run(aTick);
                if (runResult != 0){
                    if (runResult == 1){
                        sumTimeUp += (double)(i - nPrevDC) / nStepsPerYear;
            ...
            System.out.println("Num Steps: " + i);
        } catch (Exception ex){
            ex.printStackTrace();
        }
...
//        double averageTimeDown = sumTimeDown / nDCdown;
//
//        System.out.println("Ratio up: " + averageTimeUp / theoreticalTimeUP(deltaUp, mu, sigma) +
//                ", ratio down: " + averageTimeDown / theoreticalTimeDOWN(deltaDown, mu, sigma));

        double theorTimeUp = theoreticalTimeUP(deltaUp, mu, sigma);
        double theorTimeDown = theoreticalTimeDOWN(deltaDown, mu, sigma);
        double theorNumDC = theoreticalNumDC(deltaUp, deltaDown, mu, sigma);
        double averageTimeUp = sumTimeUp / nDCup;
        double averageTimeDown = sumTimeDown / nDCdown;
        double experToTheorTimeUP = averageTimeUp / theorTimeUp;
        double experToTheorTimeDOWN = averageTimeDown / theorTimeDown;

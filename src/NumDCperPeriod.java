...
public class NumDCperPeriod {
    ...
    public void run(Price aPrice){
        ...
        if (iEvent == 1 || iEvent == -1){
            ...
            if (binId == previousBinId && (dcTime - prevDCtime) < lenOfBin ){ // same bin ID in the same week
                numDCinBin += 1;
            } else {
...
//                long tempTime = prevDCtime + lenOfBin;
//                while (tempTime < dcTime){
//                    int tempBinId = findBinId(tempTime, timestampsOfBins);
//                    if (tempBinId == binId && (dcTime - tempTime) <= lenOfBin){ // we are really at exactly the same bin, not over a week, but exactly the same
//                        break;
//                    }
//                    binIndexesArray.add(tempBinId);
//                    numDCsPerBinArray.add(0);
//                    tempTime += lenOfBin;
//                }
